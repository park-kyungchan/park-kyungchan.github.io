...
const historyRoot = new URL('history/', videoRoot);
const repoRoot = new URL('../../../', import.meta.url);
const manifest = JSON.parse(readFileSync(new URL('media/manifest.json', videoRoot), 'utf8'));
const landing = readFileSync(new URL('index.html', videoRoot), 'utf8');
...
test('Pages deploys only the Final/History math-education video app at the site root', () => {
  ...
  assert.doesNotMatch(readme, /입체도형 탐구 Lab|탐구Lab Hub/);
