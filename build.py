#!/usr/bin/env python3
"""
content.json 의 텍스트를 kc_intro.html 에 박아 넣습니다(빌드).
- 편집은 content.json 만 하고, 이 스크립트를 한 번 실행하면 사이트에 반영됩니다.
- kc_intro.html 안에서 data-copy="키" 가 붙은 요소의 내용만 교체합니다(나머지는 그대로).
사용법:  python build.py
"""
import json, re, sys, os

HERE = os.path.dirname(os.path.abspath(__file__))
html_path = os.path.join(HERE, "kc_intro.html")
...
if missing:
    print("ERROR: 다음 키를 kc_intro.html 에서 찾지 못했습니다:", missing[:8])
    sys.exit(1)
...
if html != orig:
    ...
    print(f"build: kc_intro.html 갱신 완료 ({len(copy)}개 문구 적용)")
else:
